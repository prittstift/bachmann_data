...
def main():
    ...
    for result in results:
        if result.land == "land":
            result.land = "unbekannt"
        if result.land == "Deutschland (BRD)" or result.land == "Deutschland (DDR)":
            result.land = "Deutschland"
        if result.land not in labels:
            if ", " in result.land:
                split_land = result.land.split(", ")
                if split_land[0] == "Deutschland (BRD)" or split_land[0] == "Deutschland (DDR)":
                        split_land[0] = "Deutschland"
                if split_land[1] == "Deutschland (BRD)" or split_land[1] == "Deutschland (DDR)":
                        split_land[1] = "Deutschland"
