...
function compileData(error, autorData) {
    ...
    let countByLand = d3.nest()
        .key(function(datum) {
          if (datum.land.includes(",")) {
            l = datum.land.split(",")
            return l[0], l[1]}
          else {return datum.land}
          })
...
function getAutorInfo(landname, autorData){
    // Get artist name
    document.querySelector("#land-span").innerHTML = landname;
  ...
  autorData.forEach(function(autor){
        if (autor.land.includes(landname)  && autor.preis_gewonnen === "True") {
