...
def main():
    ...
    for result in results:
        if result.eingeladen_von not in labels:
            labels.append(result.eingeladen_von)
    values_priceless = []
    values_price = []
    values_bachmann = []
    for label in labels:
        ...
        k = 0
        for result in results:
            if result.eingeladen_von == label:
    ...
    for l in range(len(labels)):
        db.execute("INSERT INTO kritikerpreis (kritikerin, preis_true, preis_false, total, percent, bachmann_preis) VALUES (:kritikerin, :preis_true, :preis_false, :total, :percent, :bachmann_preis)", {
